...
function LinkItem({ to, name, icon: Icon }: { to: string; name: string; icon: any }) {
	const location = useLocation();
	const isActive = location.pathname === to;
	return (
		<NavLink to={to} className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? "text-blue-600 bg-blue-50 dark:bg-neutral-900" : "text-neutral-600 hover:text-neutral-900 dark:text-neutral-300 dark:hover:text-white"}`}>
...
export default function Navbar({ role = UserRole.REGULAR_USER, userInitials = "US" }: NavbarProps) {
