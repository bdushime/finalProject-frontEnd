export interface Equipment {
  ...
  // ---- Helpers & Mock Queries ----
  export function getEquipmentById(id: string) {
    return equipmentData.find((e) => e.id === id);
  }

  export function searchEquipment({ query, status }: { query?: string; status?: string }) {
  ...
  export function listCheckoutEligible(search: string) {
  ...
  export function listCheckoutHistory(query: string) {
    const rows = [
      { id: "H-1", item: "MacBook Pro 16\"", outAt: "2025-09-01", returnedAt: "2025-09-10", status: "Returned" },
      { id: "H-2", item: "Sony A7 III Camera", outAt: "2025-10-10", returnedAt: null as string | null, status: "On loan" },
