...
export default function ProtectedRoute({ children, allowedRoles }) {
  ...
  const handleCloseFirstTimeSetup = () => {
      ...
      {userRole === "Student" && user.mustChangePassword && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 sm:p-6 overflow-y-auto">
          <div
            ...
            className="relative w-full max-w-[min(100%,28rem)] sm:max-w-lg bg-white rounded-[20px] shadow-2xl p-6 sm:p-8 my-auto max-h-[calc(100dvh-2rem)] overflow-y-auto"
          >
            <button
              type="button"
              onClick={handleCloseFirstTimeSetup}
              className="absolute top-4 right-4 sm:top-5 sm:right-5 flex h-9 w-9 items-center justify-center rounded-full text-slate-500 hover:text-slate-800 hover:bg-slate-100 transition-colors"
            ...
            {/* Badge */}
            <div className="inline-flex items-center gap-1.5 text-[#0b1d3a] text-xs font-semibold px-3 py-1 rounded-full mb-5 bg-[#e8f0fe]">
