...
export default function ReportsPage() {
  ...
  const handleDownload = async (format) => {
    ...
    const filename = `equipment-report_${reportType}_${startDate}_${endDate}`;
  ...
  // Get categories from equipment data (simplified - in real app, fetch from API)
    ...
    <MainLayout>
      <PageContainer>
        <PageHeader
          ...
          subtitle="Generate and download customizable equipment activity reports"
        />
        ...
        {reportData.length > 0 && (
          <Card className="mb-6 border-gray-200 shadow-sm space-y-4">
            ...
            <CardContent className="p-6">
              {/* Top Result Count (Matches "Showing 10 of 14 records") */}

              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <Table className="w-full text-sm">
                  {/* Header: Light gray background, left aligned, uppercase labels */}
                  ...
                  <TableBody>
                    {currentData.map((item, idx) => (
                      <TableRow
                        key={item.id || idx}
                        // Zebra striping: even rows get a very light background
                        className="border-b border-gray-100 last:border-0 hover:bg-gray-50 even:bg-gray-50/30"
                      >
                        {getRowData(item).map((cell, cellIdx) => {
                          // Check if this is the Status column (usually the last one) to apply bold styling
                          const isStatus =
              ...
              {/* Download Section (Matches the bottom footer design) */}
