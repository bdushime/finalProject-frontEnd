                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">
                            {t('reports.subtitle', { count: filteredData.length })}
                        </p>
                    </div>
                    <button
                    ...
                    >
                        <Download className="w-4 h-4" /> {t('reports.export', 'Export Full CSV')}
                    </button>
                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-slate-50 text-slate-700 font-semibold text-xs uppercase tracking-wider border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="p-5 text-center">{t('reports.table.score')}</th> {/* New Column */}
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {filteredData.map((t) => (
                                        <tr key={t._id} className="hover:bg-slate-50/80 transition-colors">
                                            ...
                                            {/* 3. Equipment Info */}
                                            <td className="p-5">
                                                <div className="font-medium text-slate-900">{t.equipment?.name || t('equipment.dialog.deletedItem')}</div>
                                                <div className="flex gap-2 mt-1">
                                                    ...
                                                    <span className="text-xs text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded">
                                                        {t(`equipment.categories.${t.equipment?.category}`, t.equipment?.category || "General")}
                                                    </span>
                                                </div>
                                            </td>
