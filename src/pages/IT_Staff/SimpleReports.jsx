...
export default function SimpleReports() {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                ...
                setError(t('reports.error', 'Failed to load report data.'));
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleExportPDF = () => {
        if (!Array.isArray(filteredData) || filteredData.length === 0) {
            return alert(t('reports.messages.noData', 'No data to export!'));
        }
        // Export ALL filtered data, not just the page
        ...
        <ITStaffLayout>
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <PageHeader
                        title={
                            <span className="flex items-center gap-2">
                                <FileBarChart className="w-8 h-8" /> {t('reports.title', 'Reports')}
                            </span>
                        }
                    ...
                    >
                        <Download className="w-4 h-4" /> {t('reports.downloadPdf', 'Download PDF Report')}
                    </button>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative">
                        ...
                        <input
                            type="text"
                            placeholder={t('reports.searchPlaceholder', 'Search by student, email, or equipment...')}
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-slate-600 whitespace-nowrap">{t('reports.filterBy', 'Filter by:')}</span>
                        <select
                        ...
                        >
                            <option value="All">{t('equipment.categories.all', 'All Categories')}</option>
                            <option value="Laptop">{t('equipment.categories.Laptop', 'Laptop')}</option>
                            <option value="Projector">{t('equipment.categories.Projector', 'Projector')}</option>
                            <option value="Camera">{t('equipment.categories.Camera', 'Camera')}</option>
                            <option value="Audio">{t('equipment.categories.Audio', 'Audio')}</option>
                        </select>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[400px]">
                    {loading ? (
                        <div className="h-[400px] flex flex-col items-center justify-center text-[#0b1d3a] gap-3 flex-1">
                            ...
                            <p className="font-medium animate-pulse">{t('reports.loading', 'Loading...')}</p>
                        </div>
                    ) : error ? (
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center text-slate-400 gap-3 flex-1">
                            ...
                            <p>{t('reports.noRecords', 'No records found.')}</p>
                        </div>
                    ) : (
                        <>
                            <div className="overflow-x-auto flex-1">
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-slate-50 text-slate-700 font-semibold text-xs uppercase tracking-wider border-b border-slate-200">
                                        <tr>
                                            <th className="p-5">{t('reports.table.student', 'Student')}</th>
                                            <th className="p-5 text-center">{t('reports.table.score', 'Score')}</th>
                                            <th className="p-5">{t('reports.table.equipment', 'Equipment')}</th>
                                            <th className="p-5">{t('reports.table.dates', 'Dates')}</th>
                                            <th className="p-5">{t('reports.table.status', 'Status')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {/* 👇 MAP OVER paginatedData HERE with strictly 'item' */}
                                        {paginatedData.map((item) => (
                                            <tr key={item._id || Math.random()} className="hover:bg-slate-50/80 transition-colors">
                                                <td className="p-5">
                                                ...
                                                <td className="p-5 text-sm text-slate-600">
                                                    <div className="flex flex-col gap-1">
                                                        <div className="text-xs uppercase text-slate-400 font-semibold tracking-wide">{t('reports.table.borrowed', 'Borrowed')}</div>
                                                        <div>{new Date(item.createdAt).toLocaleDateString()}</div>
                                                        <div className="text-xs uppercase text-slate-400 font-semibold tracking-wide mt-1">{t('reports.table.due', 'Due')}</div>
