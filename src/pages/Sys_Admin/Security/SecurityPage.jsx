                            ...
                            <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="p-4">
                                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-bold ${
                                        log.severity === 'High' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'
                                    }`}>
                                        {log.action}
                                    </span>
                                </td>
    ...
    const renderActiveSessions = () => (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                <Activity className="w-5 h-5 text-green-500" /> Live User Sessions (24h)
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                {data.activeSessions.length === 0 && (
                    <div className="col-span-3 text-center text-gray-400 py-10">No active sessions found.</div>
                )}
            </div>
        </div>
    );
                ...
                <div className="bg-white p-2 rounded-2xl flex flex-wrap gap-2 border border-gray-100 shadow-sm">
                    {["Audit Logs", "Compliance Dashboard", "Active Sessions"].map((tab) => (
                        <button
                            ...
                            className={`px-6 py-3 rounded-xl text-sm font-bold transition-all flex-grow md:flex-grow-0 text-center ${
                                activeTab === tab
